...
### Tweaks 2016-05-24: Replaced HADegenMetric with the new
### DCRDegenMetric. Requires that metric to actually be present in the
### sims_maf version used.
...
###                sqlconstraint = 'filter = "%s"' % (ThisFilter)
        ...
        if len(LNightMax) == len(runNames):
            ...
            try:
                ...
                print "runAstromDcr.go WARN - run %i problem with NightMax" % (iRun)
                dumdum = 1.
        ...
        print "runAstromDcr.go INFO - sqlconstraint: %s ; run name %s ; nside %i" % (sqlconstraint, run, nside)
        ...
        if not os.access(outDir, os.R_OK):
            print "runAstromDcr.go INFO - creating output directory %s" % (outDir)
            os.makedirs(outDir)
...
#                                            plotFuncs=plotFuncs, \
        ...
        # difficulty.
        metric = metrics.ParallaxDcrDegenMetric(rmag=rmag, seeingCol=seeingCol, SedTemplate=SedTemplate)

        if checkCorrKind:
            metric = metrics.ParallaxDcrDegenMetric(rmag=rmag, seeingCol=seeingCol, SedTemplate=SedTemplate, useSpearmanR=useSpearmanR[iRun])
