...
# WIC - since I've made this into a standalone .py module, I think this .basemetric needs updating. 


# Also include the starcounts source
from mafContrib import starcount 


class AsTransientMetric(BaseMetric):
    """
    ...
    """
    def __init__(self, metricName='TransientDetectMetricTimesStarcount', mjdCol='expMJD',
                 ...
                 distNear=10., distFar=1000., normFac=1.0,
                 **kwargs):
        """
        ...
        distNear = closest distance, in parsecs, from which to take sources
        distFar = farthest distance, in parsecs, from which to draw sources

        """
        self.mjdCol = mjdCol
        self.m5Col = m5Col
        self.filterCol = filterCol
        super(AsTransientMetric, self).__init__(col=[self.mjdCol, self.m5Col,self.filterCol],
                                                    units='(Fraction Detected) x (StarCount)',
        ...
        self.distNear = distNear
        self.distFar = distFar
    ...
    def run(self, dataSlice, slicePoint=None):
        ...
        # WIC - compute the stellar density using Mike Lund's
        # contributed stellar density metric. 

        sliceRA = np.degrees(slicePoint['ra'])
        sliceDEC = np.degrees(slicePoint['dec'])

        numStars = starcount.starcount(sliceRA, sliceDEC, self.distNear, self.distFar)

        return float(nDetected)/nTransMax  * numStars * self.normFac
